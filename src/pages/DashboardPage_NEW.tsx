...
import { Search, FileText, CheckCircle, Clock, TrendingUp, Users } from 'lucide-react';
  ...
  const loadDashboardData = async () => {
    ...
    try {
      ...
      const patientIds = recentActivity
        ?.map((log: any) => log.metadata?.patient_id)
              ...
              {quickActions.map((action, index) => {
                ...
                return (
                  <Card
                    key={action.title}
                    className={cn(
                      'group cursor-pointer overflow-hidden border-2 border-transparent hover:border-primary-300 transition-all',
